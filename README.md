...
- 当前解析逻辑 **只支持** `hsex.men` 域名，其他站点会返回 `Invalid Host`；
- 下载逻辑假定 M3U8 中的媒体片段为 `.ts` 文件；
- 在高并发场景下，下载片段并发数默认为 `8`，可以在 `utils/dl.js` 中按实际服务器性能调整；
- 对于大文件或大量片段，下载与合并会占用一定磁盘与网络资源，请注意监控。
